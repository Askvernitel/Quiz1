...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
	...
	@Query(value = "select * from customer " +
			"where concat(concat(first_name, ' '), last_name) like :searchText", countQuery = "select count(*) from customer "
					+
					"where concat(concat(first_name, ' '), last_name) like :searchText", nativeQuery = true)
	...
	@Query("SELECT new ge.ibsu.demo.dto.GetCustomer(c.firstName, c.lastName, c.address.address, c.address.city, c.address.country) "
			+
			"FROM Customer c")
	Page<GetCustomer> searchAllCustomer(Pageable pageable);
